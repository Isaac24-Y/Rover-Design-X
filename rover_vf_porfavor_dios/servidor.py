# servidor_completo.py - Servidor mejorado con respuestas detalladas
import socket
import threading
import struct
import time
...
def procesar_comando(comando):
    """Procesa comandos del cliente con respuestas detalladas"""
    ...
    if comando.startswith("avanzar"):
        ...
        agregar_log(f"➡️ Avanzando (vel: {vel}) - {resp_arduino}")
        return f"✓ Avanzando {vel} - {resp_arduino}"
    
    elif comando.startswith("retroceder"):
        partes = comando.split()
        vel = int(partes[1]) if len(partes) > 1 else estado_rover.get("velocidad_actual", 200)
        resp_arduino = enviar_a_arduino_directo(f"retroceder {vel}")
        agregar_log(f"⬅️ Retrocediendo (vel: {vel}) - {resp_arduino}")
        return f"✓ Retrocediendo {vel} - {resp_arduino}"
    
    elif comando.startswith("izquierda"):
        partes = comando.split()
        vel = int(partes[1]) if len(partes) > 1 else estado_rover.get("velocidad_actual", 200)
        resp_arduino = enviar_a_arduino_directo(f"izquierda {vel}")
        agregar_log(f"↺ Girando izquierda (vel: {vel}) - {resp_arduino}")
        return f"✓ Izquierda {vel} - {resp_arduino}"
    
    elif comando.startswith("derecha"):
        partes = comando.split()
        vel = int(partes[1]) if len(partes) > 1 else estado_rover.get("velocidad_actual", 200)
        resp_arduino = enviar_a_arduino_directo(f"derecha {vel}")
        agregar_log(f"↻ Girando derecha (vel: {vel}) - {resp_arduino}")
        return f"✓ Derecha {vel} - {resp_arduino}"
    
    elif comando == "stop":
        resp_arduino = enviar_a_arduino_directo("stop")
        agregar_log(f"⏸ Detenido - {resp_arduino}")
    ...
    # ===== CALIBRACIÓN DE MOTORES =====
        ...
        agregar_log(f"Calibración consultada: {resp_arduino}")
    ...
    # ===== SERVOS =====
        ...
        try:
            ...
            agregar_log(f"Servo1 (codo) → {angulo}° - {resp_arduino}")
        ...
        try:
            ...
            agregar_log(f"Servo2 (sensor) → {angulo}° - {resp_arduino}")
        ...
        try:
            ...
            agregar_log(f"Servo3 (cam frontal) → {angulo}° - {resp_arduino}")
        ...
        try:
            ...
            agregar_log(f"Servo4 (cam superior) → {angulo}° - {resp_arduino}")
    ...
    # ===== MARCADORES =====
        ...
        try:
            ...
            if es_clave:
                ...
                return f"✓ MARCADOR CLAVE '{tipo}' - Secuencia autónoma iniciada"
            else:
...
if __name__ == "__main__":
    print("=" * 60)
    print("   🤖 SERVIDOR ROVER - VERSION MEJORADA")
    print("=" * 60)
